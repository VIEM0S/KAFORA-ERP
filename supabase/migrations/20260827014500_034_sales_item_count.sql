-- Meme classe de bug que discount_percent (migration 033) : p_item_count
-- est deja recu par pos_checkout() mais n'a jamais ete persiste — la
-- colonne "Articles" du CSV export de sales/page.tsx n'avait rien a lire.
alter table sales add column item_count numeric;
...
begin
  ...
  insert into sales (
    ...
    payment_method, offline_sync_id, stock_conflict, credit_conflict, item_count
  ) values (
    ...
    p_payment_method, p_offline_sync_id, coalesce(array_length(v_stock_conflicts, 1), 0) > 0, v_credit_conflict, p_item_count
  );
