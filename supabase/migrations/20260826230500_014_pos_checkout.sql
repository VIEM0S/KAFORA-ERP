-- Compteur de numerotation legale sequentielle (SYSCOHADA/OHADA), par tenant
-- et par exercice fiscal. Remplace le doc _counters/sales_{annee} + le
-- couple tx.get/tx.set Firestore : l'upsert ci-dessous est atomique sous
-- Postgres sans avoir besoin d'une transaction explicite pour CE seul geste.
...
revoke all on sale_counters from anon, authenticated;
